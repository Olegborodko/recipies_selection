class Parser < ApplicationController
  ...
  def parser
    ...
    ##########    INGREDIENTS     ##############
    ingr_cat_links = links_cat_of_ing
    ingredient_collection(ingr_cat_links)
    ingredient_save

    ##########    RECIPES     ##############
    rec_cat_links = links_cat_of_rec
    recipes_collection(rec_cat_links)
    recipe_save
  end
  ...
  def recipe_save
    @recipes2.each do |category, recipes_hash|
      ...
      recipes_hash.each do |recipe_name, href|
        ...
        add_ingr_to_rec(numb_of_ingr)
        @recipe.save!
      end
    end
  end

  def add_ingr_to_rec(numb_of_ingr)
  ...
  def recipes_collection(rec_cat_links)
    @recipes = Hash.new
    @recipes2 = Hash.new
  ...
  def ingredient_collection(ingr_cat_links)
